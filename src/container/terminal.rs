...
pub struct Terminal {
    terminal: Box<dyn AsRawFd>,
    current_termios: Termios,
    orig_termios: Termios,
}

impl Terminal {
    pub fn new<R: AsRawFd + 'static>(terminal: R) -> Result<Self> {
    ...
    pub fn get_win_size(&self) -> Result<Winsize> {
        ...
        let res = unsafe { get_winsize(self.terminal.as_raw_fd(), &mut win_size) };
        match res {
            Ok(_) => Ok(win_size),
    ...
    pub fn set_win_size(&mut self, win_size: &mut Winsize) -> Result<()> {
        let res = unsafe { set_winsize(self.terminal.as_raw_fd(), win_size) };
        match res {
            Ok(_) => Ok(()),
