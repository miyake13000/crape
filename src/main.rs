...
use clap::Parser;
use crape::args::{Action, Args, RunArgs};
use crape::container::Container;
use crape::image::HostImage;
use crape::vcs::{Commit, CommitsIter, Error, VCS};
...
const CRAPE_HOME_DIR_NAME: &str = ".crape";
const COMMITS_FILE_NAME: &str = "commits";
...
fn run(args: Args) -> Result<()> {
    if !is_root() {
        bail!("You must be root");
    }

    let home_dir = env::var("HOME")?;
    let rootdir = PathBuf::from(home_dir).join(CRAPE_HOME_DIR_NAME);

    let commits_file = rootdir.join(COMMITS_FILE_NAME);
    let mount_point = rootdir.join(MOUNTPOINT_DIR_NAME);
    let upperdir = rootdir.join(UPPER_DIR_NAME);
    let workdir = rootdir.join(WORK_DIR_NAME);
    let lower_root = rootdir.join(LOWER_DIR_NAME);
    let tmpdir = rootdir.join(TMP_DIR_NAME);
    ...
    let mut vcs = match VCS::new(&commits_file) {
        Ok(vcs) => vcs,
        Err(Error::NotInitialized) => bail!("You have to initialize with 'crape init'"),
        Err(e) => Err(e)?,
    };
